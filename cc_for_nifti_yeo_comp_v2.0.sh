...
ComparisonConditions=($Output'/'7_C_Yeo.nii)
        ...
        for CN in "${NoCs[@]}"; do
            ...
            for Condition in "${Conditions[@]}"; do
            	...
            	for ComparisonCondition in "${ComparisonConditions[@]}"; do
                    ...
                    fslcc -p 3 -t $Threshold $Source"/Melodic_ICA_"$CN"C_GSRemoved/Melodic_ICA_"??*"_"$Condition"/melodic_IC.nii.gz" $ComparisonConditions > $Output"/CC_for_"$CN"C_"$Condition"_vs_Yeo_threshold_"$Threshold.txt
...
mkdir -v $Output'/'Yeo_2011_Correlations
mv $Output'/'*Yeo*.txt $Output'/'Yeo_2011_Correlations
